...
appliedAnswer document applyExpr attr
    ...
    | "tryEval" `isInfixOf` applyExpr = Right (encodeValue (toJSON (map (outPathOf (documentOutPaths document)) (idsIn applyExpr))))
  ...
  where
    ...
    outPathOf mapping id_ = maybe Null String (Map.lookup id_ mapping)
